...
for (let index in dezDaysList) {
  ...
  //addDay.style.color = 'rgb(255,255,255)';
...
function atributeColor() {
  let diasButton = document.querySelector("#days");
  let selectedTask = document.getElementsByClassName('selected');
  let unselectedTask = document.getElementsByClassName('task');
  let unselectedTaskColor = unselectedTask[0].style.backgroundColor;
  diasButton.addEventListener('click', function(evento) {
    let corDoEvento = evento.target.style.color;
    if (evento.target.style.color !== unselectedTaskColor && unselectedTask[0].className === 'task selected') {
      let selectedTaskColor = selectedTask[0].style.backgroundColor;
      evento.target.style.color = selectedTaskColor;
    } else {
